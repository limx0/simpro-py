...
class UserTokenAuth:
    def __init__(self, host, client_key, client_secret, cache_auth=True, cache_file='auth.pkl'):
        self.host = host
        self.client_key = client_key
        self.client_secret = client_secret
        if cache_auth:
            if os.path.exists(cache_file):
                self.auth = self.load_pickle_auth(cache_file)
            else:
                self.auth = self.get_user_token_auth()
                self.save_auth(cache_file)
        else:
            self.auth = self.get_user_token_auth()
